...
class CustomUserCreationForm(UserCreationForm):
  ...
  date_birth = forms.DateField(required=True, label='Date of birth', input_formats=settings.DATE_INPUT_FORMATS, widget=forms.SelectDateWidget(years=[x for x in range(datetime.datetime.now().year, 1900-1, -1)], attrs={'class':'form-control', 'type':'date', 'format': 'yyyy-mm-dd'}))
  ...
  tel_number = forms.IntegerField(required=False, label='Phone number', widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '00421 900 123 456'}),)
